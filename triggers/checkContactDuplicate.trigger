trigger checkContactDuplicate on Contact (before insert) 
{
    ...
    {           
        ...
        {               
            ...
            if(con.FirstName == existingContact.FirstName && con.Phone != null && con.Phone == existingContact.Phone && UserInfo.getFirstName() != 'Loanpath' && UserInfo.getLastName() != 'Integration')//Bypass this check for LoanPath Application : Req from Eric, 24th May 2013
            {                                                                        
                con.addError('A possible duplicate was found.  The Name and Phone matche another record in the system');                               
            }//end-if
        }//end-if
        
        if(mapEmailContact.containsKey(con.Email) && UserInfo.getFirstName() != 'Loanpath' && UserInfo.getLastName() != 'Integration')//Bypass this check for LoanPath Application : Req from Eric, 14th May 2013
        {               
    ...
    for(List<Contact> lstExistingContacts : [SELECT id, FirstName, LastName, Phone, Email  
                                             ...
                                             WHERE (LastName IN: mapLastNameContact.keySet() AND 
                                                   ...
                                                   (email IN: mapEmailContact.keySet() AND email != null)])
    {                        
        ...
        {           
            ...
            if(mapLastNameContact.containsKey(existingContact.LastName))
            {
                ...
                if(currentContact.FirstName == existingContact.FirstName && currentContact.Phone != null && currentContact.Phone == existingContact.Phone && UserInfo.getFirstName() != 'Loanpath' && UserInfo.getLastName() != 'Integration')//Bypass this check for LoanPath Application : Req from Eric, 24th May 2013
                {                      
            ...
            if(mapEmailContact.containsKey(existingContact.Email)  && UserInfo.getFirstName() != 'Loanpath' && UserInfo.getLastName() != 'Integration')//Bypass this check for LoanPath Application : Req from Eric, 14th May 2013
            {
